...
public class ConexaoMySql {
    ...
    private final static String URL = "jdbc:mysql://127.0.0.1/locadora_terceiro_subsequente"; //endereço da base de dados do seu projeto, deve ser a mesma configurada lá no SGBD
    ...
    public static boolean createConnection() {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "Classe Driver não localizada! Talvez a biblioteca JDBC para MySQL não esteja adicionada ao projeto."); 
            result = false;
        } catch (SQLException sqlException) {
            JOptionPane.showMessageDialog(null, "Não foi possível conectar-se ao SGBD! Verifique se o SGBD está parado. Se estiver, dê start no serviço. Talvez o usuário, senha ou nome da base de dados, não estão corretos.");
            result = false;
        }
        return result;
    }
    ...
    public static boolean selecionar(String sql) {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "Não foi possível listar os registros! Talvez esteja ocorrendo erro de sintaxe script SQL, ou no nome da tabela.");
            result = false;
        }
        return result;
    }
    ...
    public static boolean atualizar(String sql) {
        ...
        try {
            ...
            if (sqlException.getErrorCode() == 1451) { //se for erro/bloqueio de chave estrangeira
               JOptionPane.showMessageDialog(null, "Não é possível executar esta ação! Esse registro faz parte de outro registro em outra tabela. Para tentar resolver exclua todos os outros registros associados com este.");
            }else{
               JOptionPane.showMessageDialog(null, "Não é possível executar esta ação! Talvez exista algum problema no script SQL. Revise-o!");
            }
            result = false;
        }
        return result;
