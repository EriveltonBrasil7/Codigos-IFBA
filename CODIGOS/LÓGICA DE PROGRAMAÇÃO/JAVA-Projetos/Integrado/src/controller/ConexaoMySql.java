...
public class ConexaoMySql {
    ...
    private final static String URL = "jdbc:mysql://127.0.0.1/biblioteca_integrado"; //endereço da base de dados do seu projeto, deve ser a mesma configurada lá no SGBD
    private final static String USUARIO = "root"; //usuário de acesso ao SGBD
    private final static String SENHA = ""; //senha do usuário acima
    /**
     * Classe-variável que receberá o objeto da conexão com o banco (possui o método de
     * conexão)
     */
    ...
    public static boolean createConnection() {
        ...
        try {
            ...
            //Se acontecer algum erro ele abre a tela DialogError, e preenche essa tela com as informações do ClassNotFoundException
            DialogError dialog = new DialogError(null, true);
            dialog.setLabelTipoErro("Classe Driver não localizada!");
            dialog.setLabelDicaErro1("Talvez a biblioteca JDBC para MySQL não esteja adicionada ao projeto.");
            dialog.setLabelDicaErro2("Contate o administrador do sistema e envie as menssagens de erro abaixo:");
            dialog.setAreaTexto("ERROR CAUSE: " + classNotFoundException.getMessage() + "\n\nSTACK TRACE:\n" + classNotFoundException);
            dialog.setVisible(true);
            result = false;
        } catch (SQLException sqlException) {
            //Se acontecer algum erro ele abre a tela DialogError, e preenche essa tela com as informações do SQLException
            DialogError dialog = new DialogError(null, true);
            dialog.setLabelTipoErro("Não foi possível conectar-se ao SGBD!");
            dialog.setLabelDicaErro1("Verifique se o SGBD está parado. Se estiver, dê start no serviço.");
            dialog.setLabelDicaErro2("Talvez o usuário, senha ou nome da base de dados, não estão corretos.");
            dialog.setAreaTexto("ERROR CODE: " + sqlException.getErrorCode() + "\n\nSTACK TRACE:\n" + sqlException.toString());
            dialog.setVisible(true);
            result = false;
        }
        return result;
     ...
     * <p>
     * O método createStatement() - É usado para criar um objeto Statement, com
     * este objeto, é possível executar instruções SQL ao SGBD.
     * <p>
    ...
    public static boolean selecionar(String sql) {
        ...
        try {
            ...
            //Se acontecer algum erro ele abre a tela DialogError, e preenche essa tela com as informações do SQLException
            DialogError dialog = new DialogError(null, true);
            dialog.setLabelTipoErro("Não foi possível listar os registros!");
            dialog.setLabelDicaErro1("Talvez esteja ocorrendo erro de sintaxe script SQL, ou no nome da tabela.");
            dialog.setLabelDicaErro2("Contate o administrador do sistema e envie as menssagens de erro abaixo:");
            dialog.setAreaTexto("ERROR CODE: " + sqlException.getErrorCode() + "\n\nSCRIPT SQL:\n" + sql + "\n\nSTACK TRACE:\n" + sqlException.toString());
            dialog.setVisible(true);
            result = false;
    ...
    public static boolean atualizar(String sql) {
        ...
        try {
            ...
            if (sqlException.getErrorCode() == 1451) { //se for erro/bloqueio de chave estrangeira
                dialog.setLabelTipoErro("Não é possível executar esta ação!");
                dialog.setLabelDicaErro1("Esse registro faz parte de outro registro em outra tabela.");
                dialog.setLabelDicaErro2("Para tentar resolver exclua todos os outros registros associados com este.");
                dialog.setAreaTexto("ERROR CODE: " + sqlException.getErrorCode() + "\n\nSCRIPT SQL:\n" + sql + "\n\nSTACK TRACE:\n" + sqlException.toString());
                dialog.setVisible(true);
            }else{
                dialog.setLabelTipoErro("Não é possível executar esta ação!");
                dialog.setLabelDicaErro1("Talvez exista algum problema no script SQL. Revise-o!");
                dialog.setLabelDicaErro2("Contate o administrador do sistema e envie as menssagens de erro abaixo:");
                dialog.setAreaTexto("ERROR CODE: " + sqlException.getErrorCode() + "\n\nSCRIPT SQL:\n" + sql + "\n\nSTACK TRACE:\n" + sqlException.toString());
                dialog.setVisible(true);
            }
...
//    private static String scapeSql(String sql){
//        sql.
//    }
